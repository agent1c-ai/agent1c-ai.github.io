...
serve(async (req) => {
  ...
  if (error) {
    return new Response(JSON.stringify({ error: "Could not read usage", detail: error.message }), { status: 500, headers })
  }

  const used = Math.max(0, Number(row?.input_tokens || 0) + Number(row?.output_tokens || 0))
