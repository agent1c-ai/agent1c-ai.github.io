...
function activateAiIntroGuide({ setClippyMode, showClippyBubble, renderClippyBubble, setStatus, positionClippyAtRight }){
  const showGuide = () => {
    setClippyMode(true)
    if (typeof positionClippyAtRight === "function") positionClippyAtRight()
    showClippyBubble({ variant: "compact", snapNoOverlap: true, preferAbove: true })
    renderClippyBubble()
    setStatus("Choose local or cloud in the Intro window.")
  }
  if (window.__agent1cPreloadActive) {
    const once = () => {
      window.removeEventListener("agent1c:preload-finished", once)
      showGuide()
    }
    window.addEventListener("agent1c:preload-finished", once)
  } else {
    showGuide()
  }
}
