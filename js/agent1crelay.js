...
function relaySetupByOs(os){
  ...
  const healthNoToken = `curl -s -H "Origin: https://agent1c.ai" http://127.0.0.1:8765/v1/health`
  if (os === "mac") {
    return {
      label: "macOS",
  ...
  if (os === "android") {
    return {
      ...
      uninstallTitle: "Optional: uninstall relay scripts",
      uninstallCmd: "rm -rf ~/.agent1c-relay",
      caveat: "Android browsers may block HTTPS->localhost private-network requests. Ensure your browser permits local private-network CORS for agent1c.ai.",
    }
  }
  return {
    label: "Linux",
