...
- `.ai` currently has:
  ...
  - browser proxy fallback path
  - experimental web proxy toggle (shared between Shell Relay and Tor Relay)
  - CSS `url(...)` and `srcset` rewriting
  - canonical proxied link navigation (browser URL stays on target URL)
  - GET form submit bridge (including scripted `form.submit()` / `requestSubmit()`)
- `.me` should stay in lockstep when proxy changes land on `.ai`, unless a change is cloud-only.

### Regression lesson (important)

- Do not add browser-side proxy preflight that performs a second fetch before iframe load.
- Double-fetch caused regressions on some sites (e.g. Yahoo) and was reverted.
- Anti-bot detection should be implemented in a single-fetch path (relay-side signal or existing response inspection).
