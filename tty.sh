...
#
# IMPORTANT: install.sh is bash. Pipe to bash, not dash/sh — otherwise
# `[[` / `pipefail` break and the installer falsely tries sudo apt-get.
set -eu
...
if [ -r /dev/tty ] && [ -t 1 ]; then
  curl -fsSL "$INSTALL_URL" | bash -s -- "$@" </dev/tty
else
